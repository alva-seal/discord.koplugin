...
local function saveAndSendHighlightToBot(self, instance)
    logger.info("--- Entering sendHighlightToBot ---")
    -- logger.info(instance.document)
    ...
    if not instance.selected_text or not instance.selected_text.text or instance.selected_text.text == "" then
        logger.warn("Send to Bot: No text selected.")
    ...
    code = code:upper()
    logger.info("Send to Bot: Using code:", code)
    ...
    local function performSendRequest()
        logger.dbg("Send to Bot: Sending payload:", json_payload)
        ...
        if not NetworkMgr:isConnected() then
            ...
            NetworkMgr:promptWifiOn(function()
                ...
                saveAndSendHighlightToBot(self, instance)
        ...
        local co = coroutine.create(function(handler_func)
            ...
            logger.info("Send to Bot: Making HTTP request to", request_url)
            ...
            if not ok_req then
                ...
                logger.info("Send to Bot: HTTP Response Code:", response_code or "N/A")
                logger.dbg("Send to Bot: HTTP Response Body:", response_body)
            end
        ...
        local function handleResult(success, body, code)
            _hideProgress()
            if success then
                logger.info("Send to Bot: Highlight sent successfully!")
    ...
    performSendRequest()
    logger.info("--- Exiting sendHighlightToBot ---")
end
